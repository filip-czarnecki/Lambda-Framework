...
class AppverseModel extends AppverseCommon {
  ...
  public function resolveAll() {
    ...
    if(isset($module['package'])) {
      foreach($module['package'] as $package) {
        $resolve[] = $this->resolve($module);
      }
    }
    if(isset($module['dependency'])) {
      foreach($module['dependency'] as $dependency=>$action) {
        $resolve[] = $this->resolve($dependency);
      }
    }
    foreach($resolve as $res) {
      $content = $content + $this->gettemplatecontent($res);
    }
    return $content;
  }
  private function resolve($module) {
    if(!isset($this->modules)) {
      $this->modules = $this->getmodules();
    }
    if(isset($module['dependency'])) {
  ...
  public function install($result,$config,$module=null) {
    #used by Appverse->install
    if($module == null) {
      $module = $this->module;
    }
  ...
  private function add($result,$config,$module) {
    $installer = new GenericInstaller($module['module']);
    $installer->install($result,$config,$module['config']);
    return 1;
  }
